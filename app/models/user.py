...
class User(UserMixin,Base):
    ...
    def can_send_drift(self):
        if self.beans<1:
            return False
        success_gifs_count=Gift.query.filter_by(uid=self.id,launched=True).count()
        success_receive_count=Drift.query.filter_by(
            requester_id=self.id,pending=PendingStatus.Success).count()
        return True if floor(success_gifs_count/2)>=floor(success_receive_count/2) else False
